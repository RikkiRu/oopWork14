	...
	public partial class MainForm : Form {
		...
		private void bSetAnswer_Click(object sender, EventArgs e) {
			if (CurrentQA != null) {
				...
				var queryResult = service.answerQA(CurrentQA);
				if (queryResult.CompareTo("Отправлено") == 0) {
					ReportCreator.CreateQuestionReport(service.getFirmInfo(), currentConsulter.Firstname + ' ' + currentConsulter.Lastname, CurrentQA, this.service.getThemes().Where(theme => theme.ID == CurrentQA.ThemeID).First().Theme);
				}
